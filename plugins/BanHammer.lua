...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '> ['..user_id..' ] Banned for all <b>M A X I M U S +</b> Groups/SuperGroups! (Globally banned)')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '> ['..user_id..' ] Unbanned for all <b>M A X I M U S +</b> Groups/SuperGroups! (Unglobally banned)')
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '> ['..user_id..' ] Banned for all <b>M A X I M U S +</b> Groups/SuperGroups! (Globally banned)'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '> ['..user_id..' ] Unbanned for all <b>M A X I M U S +</b> Groups/SuperGroups! (Unglobally banned)'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
